...
MCSRLr <- function(n, p, rho, beta_0) {
  ...
  MSE10 <- numeric(num_replications)
  MSE11 <- numeric(num_replications)
  
  
  for (r in 1:num_replications) {
    # Generate data
    B <- rep(sqrt(1 / p), p)
    w <- rnorm(n * p) # Generate the 200 + n*p random numbers
    z <- matrix(w, n, p) # Disregard the first 200 random numbers in w 
    ...
    MSE_values <- c(MSE0, MSE1, MSE2, MSE3, MSE4, MSE5, MSE6, MSE7, MSE8,MSE9,MSE10,MSE11 )  # Combine all MSE results
    ...
    k9 <- 0.4512171333234585*k6+0.269033582616763*k7+0.27974928405978*k8
    k10 <- s(-2.714*h(-.155*n-.241*p+.306*k1-.372*k2+.333)-.410)
    k11 <-.279773*k5+.274717*k4+.114068*k6+.111779*k9+.109964*k8+.109698*k7
    ...
    beta_k10 <- logistic_ridge_regression(X, y, k10)
    beta_k11 <- logistic_ridge_regression(X, y, k11)
    ...
    MSE10[r] <- sum((beta_k10 - BTrue)^2)
    MSE11[r] <- sum((beta_k11 - BTrue)^2)
    
  }
  
  # Summary statisticsS
  summary_stats <- data.frame(Lambda = c("k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7", "k8","k9","k10","k11"),
                              Min_MSE = c(min(MSE0, na.rm = TRUE), min(MSE1, na.rm = TRUE), min(MSE2, na.rm = TRUE), min(MSE3, na.rm = TRUE), min(MSE4, na.rm = TRUE), min(MSE5, na.rm = TRUE), min(MSE6, na.rm = TRUE), min(MSE7, na.rm = TRUE), min(MSE8, na.rm = TRUE),min(MSE9, na.rm = TRUE),min(MSE10, na.rm = TRUE),min(MSE11, na.rm = TRUE)),
                              Max_MSE = c(max(MSE0, na.rm = TRUE), max(MSE1, na.rm = TRUE), max(MSE2, na.rm = TRUE), max(MSE3, na.rm = TRUE), max(MSE4, na.rm = TRUE), max(MSE5, na.rm = TRUE), max(MSE6, na.rm = TRUE), max(MSE7, na.rm = TRUE), max(MSE8, na.rm = TRUE),max(MSE9, na.rm = TRUE),max(MSE10, na.rm = TRUE),max(MSE11, na.rm = TRUE)),
                              Mean_MSE = c(mean(MSE0, na.rm = TRUE), mean(MSE1, na.rm = TRUE), mean(MSE2, na.rm = TRUE), mean(MSE3, na.rm = TRUE), mean(MSE4, na.rm = TRUE), mean(MSE5, na.rm = TRUE), mean(MSE6, na.rm = TRUE), mean(MSE7, na.rm = TRUE), mean(MSE8, na.rm = TRUE),mean(MSE9, na.rm = TRUE), mean(MSE10, na.rm = TRUE),mean(MSE11, na.rm = TRUE)),

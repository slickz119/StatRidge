...
MCSRLr <- function(n, p, rho, beta_0) {
  ...
  for (r in 1:num_replications) {
    ...
    MSE_values <- c(MSE0, MSE1, MSE2, MSE3, MSE4, MSE5, MSE6, MSE7, MSE8,MSE9,MSE10 )  # Combine all MSE results
    ...
    k9 <- 0.368184125*k6+0.262393398*k7+0.269684641*k8
    k10 <- s(-2.009*s(.333*k1-.315*k2 -3.371*k3-1.791*k4-2.797*k5 +1.107*k6+1.019*k7+.249*k8-.237*k9+2.354)+.090)
  ...
  # Summary statisticsS
  summary_stats <- data.frame(Lambda = c("k0", "k1", "k2", "k3", "k4", "k5", "k6", "k7", "k8","k9","k10"),
                              Min_MSE = c(min(MSE0, na.rm = TRUE), min(MSE1, na.rm = TRUE), min(MSE2, na.rm = TRUE), min(MSE3, na.rm = TRUE), min(MSE4, na.rm = TRUE), min(MSE5, na.rm = TRUE), min(MSE6, na.rm = TRUE), min(MSE7, na.rm = TRUE), min(MSE8, na.rm = TRUE),min(MSE9, na.rm = TRUE),min(MSE10, na.rm = TRUE)),
                              Max_MSE = c(max(MSE0, na.rm = TRUE), max(MSE1, na.rm = TRUE), max(MSE2, na.rm = TRUE), max(MSE3, na.rm = TRUE), max(MSE4, na.rm = TRUE), max(MSE5, na.rm = TRUE), max(MSE6, na.rm = TRUE), max(MSE7, na.rm = TRUE), max(MSE8, na.rm = TRUE),max(MSE9, na.rm = TRUE),max(MSE10, na.rm = TRUE)),
                              Mean_MSE = c(mean(MSE0, na.rm = TRUE), mean(MSE1, na.rm = TRUE), mean(MSE2, na.rm = TRUE), mean(MSE3, na.rm = TRUE), mean(MSE4, na.rm = TRUE), mean(MSE5, na.rm = TRUE), mean(MSE6, na.rm = TRUE), mean(MSE7, na.rm = TRUE), mean(MSE8, na.rm = TRUE),mean(MSE9, na.rm = TRUE), mean(MSE10, na.rm = TRUE)),
